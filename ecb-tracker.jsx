...
export default function App() {
  ...
  const addEntry       = (e) => setData(d=>({...d,entries:[...d.entries,e]}));
    ...
    <>
      ...
      <div className="shell">
        ...
        <main className="main">
          ...
          {page==="settings"  && <PageSettings settings={data.settings} updateSettings={updateSettings} showToast={showToast} onClear={()=>{setData(INIT);showToast("All data cleared","warn");}} />}
        </main>
      </div>
    </>
  );
...
function PageLog({ entries, rate, addEntry, showToast }) {
  ...
  const handleImg = (e) => {
    ...
    reader.onload=()=>setForm(f=>({...f,imgData:reader.result,imgName:file.name}));
  ...
  const submit = () => {
    ...
    addEntry({...form, id:Date.now(), unit:parseFloat(form.unit)});
